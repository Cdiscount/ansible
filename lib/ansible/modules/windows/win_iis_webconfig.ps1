...
function Cast-PropertyValue
{
    ...
    if ($Property | Get-Member -Name "Value") {
        $RefValue = $Property.Value
    } else {
        $RefValue = $Property
    }

    if ($RefValue -is [byte] -or $RefValue -is [int16] -or $RefValue -is [int32] -or $RefValue -is [int64] `
           -or $RefValue -is [sbyte] -or $RefValue -is [uint16] -or $RefValue -is [uint32] -or $RefValue -is [uint64] `
           -or $RefValue -is [float] -or $RefValue -is [double] -or $RefValue -is [decimal]) {
        $Value -as $RefValue.GetType()
    } else {
        $Value
    }
}
...
if ($type -eq "property" -and $value -ne $null) {
    ...
    if (-not (Compare-PropertyValue -Property $property -Value $value)) {
        try {
            $arguments.Add("Value", ($value | Cast-PropertyValue -Property $property))
    ...
    if ($state -eq "absent") {
        if ($propertyItem -ne $null) {
            ...
            try {
                ...
                $arguments.Add("AtElement", @{$itemKey = ($itemValue | Cast-PropertyValue -Property $propertyItem.GetAttribute($itemKey))})
            ...
            foreach ($key in $attributes.Keys) {
                if ($attributes.$key -ne $propertyItem.$key) {
                    ...
                    try {
                        ...
                        $arguments.Name = $key
                        $arguments.Add("Value", ($attributes.$key | Cast-PropertyValue -Property $propertyItem.GetAttribute($key)))
